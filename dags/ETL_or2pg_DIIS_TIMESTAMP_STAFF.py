...
TABLE_CONFIGS = [
    {
        "source_sql": "SELECT * FROM eis.DSS_PERSON65_V_STAFF_NOTWORK_66",
        "target_table": "public.DSS_PERSON65_V_STAFF_NOTWORK_66",
        "mode": "replace"  # หรือ "append"
    },
    {
        "source_sql": "SELECT * FROM eis.TIMESTAMP_V_TIMESTAMP_B3",
        "target_table": "public.TIMESTAMP_V_TIMESTAMP_B3",
        "mode": "replace"  # หรือ "append"
    },
...
def extract_and_load_table(source_sql, target_table, mode='replace'):
    ...
    postgres_hook = PostgresHook(postgres_conn_id='Postgres_DIIS')
...
default_args = {
    'owner': 'nattapon',
    'start_date': datetime(2025, 7, 1, tzinfo=tz),  # เวลาเริ่มแบบไทย
    'depends_on_past': False,
}

with DAG(
    dag_id='ETL_or2pg_DIIS_TIMESTAMP_STAFF',
    default_args=default_args,
    schedule_interval='0 1 * * *',  # ✅ รันทุกวันเวลา 01:00
    catchup=False,
    tags=['DIIS', 'oracle', 'postgres', 'etl'],
    ...
    dag.doc_md = """
        ...
        # -✅ รันทุกวันเวลา 01:00 น. (Asia/Bangkok)
        # ---------------------------------------------------------------
    """
